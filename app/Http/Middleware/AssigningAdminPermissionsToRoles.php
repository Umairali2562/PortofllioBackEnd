...
class AssigningAdminPermissionsToRoles
{
    /**
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($authenticatedUser && !$authenticatedUser->roles->pluck('slug')->contains(function ($slug) {
                return preg_match("/\badministrator\b/i", $slug);
            })){
            ...
            if (!is_null($permissionsArr)) {
                ...
                foreach ($permissions as $permission) {
                    if (preg_match("/\bAdmin\b|\bAdministrator\b|\bmanage-permissions\b/i", $permission)) {
